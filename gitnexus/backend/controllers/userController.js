...
// *project*/backend/controllers/userController.js
// Временная версия loginUser с дополнительной отладкой
...
async function loginUser(req, res, next) {
  try {
    ...
    const existing = await User.findByUsernameOrEmail(usernameOrEmail);
    console.log('User found:', existing ? 'Yes' : 'No');

    // 3. Если нет пользователя
    if (!existing) {
    ...
    // 5. Проверяем наличие SECRET_KEY
    if (!SECRET_KEY) {
