...
class LoyaltyService {
  ...
  static async processReferral(referrerId, referredId) {
    ...
    if (referrer.user_id === newUserId) {
      throw new BusinessError('Cannot use your own referral code');
    }
  ...
  // Обновить claimAchievement:
  static async claimAchievement(userId, achievementId) {
    // Проверяем условия достижения
    const conditions = await this.checkAchievementConditions(userId, achievementId);

    if (!conditions.met) {
      throw new BusinessError('Achievement conditions not met', conditions);
    }

    // Разблокируем достижение
    await UserLoyaltyModel.unlockAchievement(userId, achievementId, {
    ...
    // Начисляем награду
    if (conditions.reward > 0) {
      await UserLoyaltyModel.addPoints(userId, conditions.reward, `achievement_${achievementId}`);
    }

    return await UserLoyaltyModel.findByUserId(userId);
  }
