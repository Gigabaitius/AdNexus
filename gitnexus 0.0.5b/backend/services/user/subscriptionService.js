...
class SubscriptionService {
  ...
  static async upgradeSubscription(userId, plan) {
    const pricing = {
      'standard': { price: 9.99, days: 30 },
      'premium': { price: 29.99, days: 30 }
    };
    ...
    // Проверяем баланс
    const finance = await UserFinanceModel.findByUserId(userId);
    if (finance.balance < planData.price) {
      throw new BusinessError('Insufficient funds for subscription');
    }

    return await UserFinanceModel.transaction(async () => {
      // Списываем средства
      await UserFinanceModel.convertHeldToSpent(
        userId,
        planData.price,
      ...
      // Обновляем подписку
      const expiresAt = new Date(Date.now() + planData.days * 24 * 60 * 60 * 1000);
      await UserFinanceModel.updateSubscription(userId, plan, expiresAt);

      // Начисляем бонусные поинты
      const bonusPoints = plan === 'premium' ? 100 : 50;
      await UserLoyaltyModel.addPoints(userId, bonusPoints, 'subscription_upgrade');

      return await UserFinanceModel.findByUserId(userId);
    });
  }
