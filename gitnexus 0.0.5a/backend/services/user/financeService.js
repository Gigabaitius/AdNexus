...
class FinanceService {
   ...
   */
  static async deposit(userId, amount, metadata = {}) {
    if (amount <= 0) {
    ...
    await UserFinanceModel.updateStats(userId, 'total_earned', amount);
   ...
   */
  static async withdraw(userId, amount, withdrawalData) {
    if (amount <= 0) {
      throw new Error('Amount must be positive');
    }
    ...
    await UserFinanceModel.updateStats(userId, 'total_withdrawn', amount);
